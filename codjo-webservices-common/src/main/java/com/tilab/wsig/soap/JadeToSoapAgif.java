...
public class JadeToSoapAgif {

    private static Logger log = Logger.getLogger(JadeToSoapAgif.class.getName());
     ...
     * @throws Exception
     */
    public SOAPMessage convert(AbsTerm resultAbsObject, WSIGService wsigService, String operationName)
          throws Exception {
        ...
        try {
            ...
            throw new Exception("Operation schema not found for operation " + operationName + " in "
                                + onto.getName() + " ontology", oe);
        }
        ...
        if (resultSchema != null) {
            ...
            // Create soap message
            convertObjectToSoapElement(actionSchema, resultSchema, resultAbsObject,
                                       WSDLGeneratorUtils.getResultName(operationName), responseElement);
        }
        else {
     ...
     * @throws Exception
     */
    private SOAPElement convertObjectToSoapElement(ObjectSchema containerSchema,
                                                   ObjectSchema resultSchema,
                                                   AbsTerm resultAbsObj,
                                                   String elementName,
                                                   SOAPElement rootSoapElement) throws Exception {
        ...
        if (resultSchema instanceof PrimitiveSchema) {
            ...
            // PrimitiveSchema
            log.debug(
                  "Elaborate primitive schema: " + elementName + " of type: " + resultSchema.getTypeName());
            ...
            // for every other kind of object, just call toString if they are not null;
            // iotherwise set null
            // if (BasicOntology.DATE.equals(primitiveAbsObj.getTypeName())) {

            if (primitiveAbsObj != null) {
                if (BasicOntology.DATE.equals(resultSchema.getTypeName())) {
...
//                soapElement.addTextNode(null);
                // otherwise set default value fot the given type
                if (BasicOntology.DATE.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode(null);
                }
                else if (BasicOntology.STRING.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode("");
                }
                else if (BasicOntology.FLOAT.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode("0.0");
                }
                else if (BasicOntology.INTEGER.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode("0");
                }
                else if (BasicOntology.BOOLEAN.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode("false");
                }
                else if (BasicOntology.BYTE_SEQUENCE.equals(resultSchema.getTypeName())) {
                    soapElement.addTextNode("");
                }
            }
        }
            ...
            for (String conceptSlotName : resultSchema.getNames()) {
                ...
                AbsTerm subAbsObject;

                if (resultAbsObj != null) {
                    subAbsObject = (AbsTerm)resultAbsObj.getAbsObject(conceptSlotName);
                }
                else {
                    subAbsObject = null;
                }
                ...
                // Do recursive call
                convertObjectToSoapElement(newContainerSchema, slotSchema, subAbsObject, conceptSlotName,
                                           soapElement);
            }
        }
            ...
            if (aggregateAbsObj != null) {
                for (int i = 0; i < aggregateAbsObj.size(); i++) {
                    ...
                    // Do ricorsive call
                    convertObjectToSoapElement(newContainerSchema, aggrSchema, itemObject, itemName,
                                               soapElement);
                }
            }
        }
     ...
     * @throws Exception
     */
    private SOAPElement addSoapElement(SOAPElement rootSoapElement,
                                       String elementName,
                                       String uri,
                                       String soapType) throws Exception {
